...
pub struct UXAddManyGate<F: RichField + Extendable<D>, const D: usize, const BITS: usize> {
    pub num_addends: usize,
    pub num_ops: usize,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, const BITS: usize> UXAddManyGate<F, D, BITS> {
...
#[derive(Clone, Debug)]
struct UXAddManyGenerator<F: RichField + Extendable<D>, const D: usize, const BITS: usize> {
    gate: UXAddManyGate<F, D, BITS>,
    row: usize,
    i: usize,
    _phantom: PhantomData<F>,
}
...
impl<F: RichField + Extendable<D>, const D: usize, const BITS: usize> SimpleGenerator<F, D>
    for UXAddManyGenerator<F, D, BITS>
{
    fn id(&self) -> String {
        "UXAddManyGenerator".to_string()
    }
    ...
    fn deserialize(src: &mut Buffer, common_data: &CommonCircuitData<F, D>) -> IoResult<Self> {
        let gate = UXAddManyGate::deserialize(src, common_data)?;
    ...
    fn run_once(
        ...
        let output_carry_u64 = output_u64 >> BITS;
        let output_result_u64 = output_u64 & ((1u64 << BITS) - 1);
        ...
        let num_result_limbs = UXAddManyGate::<F, D, BITS>::num_result_limbs();
...
mod tests {
    ...
    #[test]
    fn low_degree() {
        test_low_degree::<GoldilocksField, _, 4>(UXAddManyGate::<GoldilocksField, 4, 32> {
            num_addends: 4,
            num_ops: 3,
            _phantom: PhantomData,
        })
    ...
    fn eval_fns() -> Result<()> {
        ...
        test_eval_fns::<F, C, _, D>(UXAddManyGate::<GoldilocksField, D, 32> {
            num_addends: 4,
            num_ops: 3,
            _phantom: PhantomData,
        })
    ...
    fn test_gate_constraint() {
        ...
        fn get_wires(addends: Vec<Vec<u64>>, carries: Vec<u64>) -> Vec<FF> {
            ...
            let num_result_limbs = UXAddManyGate::<F, D, BITS>::num_result_limbs();
            let num_carry_limbs = UXAddManyGate::<F, D, BITS>::num_carry_limbs();
            let limb_base = 1 << UXAddManyGate::<F, D, BITS>::limb_bits();
        ...
        let addends: Vec<Vec<_>> = (0..NUM_U32_ADD_MANY_OPS)
            .map(|_| (0..NUM_ADDENDS).map(|_| rng.gen::<u64>() % modd).collect())
            .collect();
        let carries: Vec<_> = (0..NUM_U32_ADD_MANY_OPS)
            .map(|_| rng.gen::<u64>() % modd)
            .collect();

        let gate = UXAddManyGate::<F, D, BITS> {
